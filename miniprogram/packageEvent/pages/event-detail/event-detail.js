...
Page({
  ...
  onShow() {
    if (this._eventId && this.data.pageState === 'ready') {
  ...
  refreshRegistrationState() {
    var self = this;
    if (!auth.isLoggedIn()) {
      self.applyRegistrationState(false);
      return;
    }
    eventService.hasRegisteredCompetition(this._eventId).then(function (registered) {
      self.applyRegistrationState(registered);
    });
  },
  ...
  applyRegistrationState(registered) {
    this.setData({
      ...
      registerBtnText: registered ? '已报名' : '赛事报名',
      registerDisabled: !!registered
    });
  },
  ...
  onGoRegister() {
    ...
    if (this.data.hasRegistered || this.data.registerDisabled) {
